...
export default class EnterSalaryController {
  ...
  private async createCreditCardBillItems(
    currentMonth: number,
    transaction: any
  ...
  private async findAllCompulsionsResumes(transaction: any) {
    ...
    if (
      !responseResumesDatabase.results ||
      responseResumesDatabase.results.length === 0
    ) {
      span.finish();
      console.log("--------------------");
  ...
  private async updateCompulsionWithSalary(
    resumeId: string,
    currentMonth: number,
    salaryId: string
  ) {
  ...
  private async updateCompulsionsResumesWithCorrectSalary(
    itemsCompulsionsResumes: any[],
    currentMonth: number,
    salaryId: string,
    transaction: any,
    pastSalaryIds: PastSalaryIds,
    pastMonths: PastMonths
  ) {
    const span = transaction.startChild({
      op: "updateCompulsionsResumesWithCorrectSalary",
    });
    console.log("--------------------");
    ...
    try {
      ...
      for (const resumeItem of itemsCompulsionsResumes) {
        if (resumeItem.properties["Current Month Number"] != currentMonth) {
          pastSalaryIds[resumeItem.id] =
            resumeItem.properties["Salário"].relation[0].id;
          pastMonths[resumeItem.id] =
            resumeItem.properties["Current Month Number"].number;
          const updateResponse = await this.updateCompulsionWithSalary(
            resumeItem.id,
            currentMonth,
            salaryId
          );
  ...
  private async fillWhatLeftFromPastMonth(
    currentMonth: number,
    transaction: any
  ) {
    ...
    try {
      ...
      if (
        !searchPastMonthResponse ||
        !searchPastMonthResponse.results ||
        searchPastMonthResponse.results.length === 0
      ) {
        span.finish();
        console.log("--------------------");
        throw new Error(
          "Erro ao buscar mes passado para preencher valor que sobrou!"
        );
      }

      const pastMonthItem = searchPastMonthResponse.results[0];

      const whatLeftFromPastMonth =
        pastMonthItem.properties["Quanto vai sobrar"].formula.number;
  ...
  async doEnterSalary(req: any, res: any) {
    ...
    try {
      ...
      const [bill1Id, bill2Id] = await this.createCreditCardBillItems(
        currentMonth,
        transaction
      );
      ...
      const salaryId = await this.createSalaryItem(
        req.body.salary,
        currentMonth,
        bill1Id,
        bill2Id,
      ...
      const itemsCompulsionsResumes = await this.findAllCompulsionsResumes(
        transaction
      );
      ...
      await this.updateCompulsionsResumesWithCorrectSalary(
        itemsCompulsionsResumes,
        currentMonth,
        salaryId,
        transaction,
        pastSalaryIds,
      ...
      try {
        ...
        try {
          if (
            dataToDelete["pastSalaryIds"] &&
            Object.keys(dataToDelete["pastSalaryIds"]).length > 0
          ) {
            for (const itemToUndo in dataToDelete[
              "pastSalaryIds"
            ] as PastSalaryIds) {
              const pastSalaryId = (
                dataToDelete["pastSalaryIds"] as PastSalaryIds
              )[itemToUndo];

              if (
                !dataToDelete["pastMonths"] ||
                !(dataToDelete["pastMonths"] as PastMonths)[itemToUndo]
              ) {
              ...
              const responseDelete5 = await this.updateCompulsionWithSalary(
                itemToUndo,
                (dataToDelete["pastMonths"] as PastMonths)[itemToUndo],
                pastSalaryId
              );
